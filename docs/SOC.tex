...
Lambda funkce začíná slovem \texttt{fun}. Následují argumenty funkce, výraz specifikující typ navrácené hodnoty a blok kódu, který tvoří několik příkazů ve složených závorkách. Argumenty funkce se zapisují v hranatých závorkách.
\begin{grammar}
<lambda> ::= \emphasis{fun} <arguments> <expression> <block>

<arguments> ::= \emphasis{[} <argument-group>* \emphasis{]}

<argument-group> ::= <name>+ \emphasis{:} <expression>
