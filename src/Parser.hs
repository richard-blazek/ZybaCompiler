...
data Expression
  ...
  | Access (Integer, Expression) String
...
parseValue ((line, Lexer.LiteralInt _ lit) : tokens) = Right ((line, Literal $ Int lit), tokens)
parseValue ((line, Lexer.LiteralReal _ n exp) : tokens) = Right ((line, Literal $ Real $ fromIntegral n / (10.0 ** fromIntegral exp)), tokens)
parseValue ((line, Lexer.LiteralText lit) : tokens) = Right ((line, Literal $ Text lit), tokens)
parseValue ((line, Lexer.LiteralBool lit) : tokens) = Right ((line, Literal $ Bool lit), tokens)
...
parseValue ((line, token) : _) = err line $ "Expected a value but got " ++ show token
...
parseRecord line tokens = do
  ...
  Right ((line, Record fields), tokensAfterFields)
