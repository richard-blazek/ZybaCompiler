...
test1 = testEqual "Analysing the first code" [
  ("heron", Function (naryFunction 3) ["a", "b", "c"] $ s \* (s \- arg "a") \* (s \- arg "b") \* (s \- arg "c")),
  ("fact", Function (naryFunction 1) ["n"] $
...
test2 = testEqual "Analysing the second code" [
  ("pow", Function (naryFunction 2) ["n", "e"] $ call "power" [arg "n", arg "e", int 1]),
  ("power", Function (naryFunction 3) ["n", "exp", "acc"] $
